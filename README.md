# AL AZEEM KIRANA AND GENERAL STORE

### Wholesale · Retail · Online Ordering

A full-stack e-commerce platform developed for AL AZEEM KIRANA AND GENERAL STORE, allowing customers to browse products, place orders, and make payments while the store owner manages products, inventory, and orders through an admin dashboard.

## 🌐 Live Demo

[Visit AL AZEEM KIRANA AND GENERAL STORE](https://al-azeem-store.vercel.app/)

## 📌 About the Project

AL AZEEM KIRANA AND GENERAL STORE is a complete online shopping platform designed for a wholesale and retail grocery business.

The application includes a customer-facing store, secure authentication, product management, inventory tracking, order management, and cloud database integration.

It is deployed online using Vercel, with Supabase used for cloud data storage.

## ✨ Key Features

### Customer Features

- Browse products by category
- Search for products
- Add products to cart
- Update cart quantities
- Customer registration and login
- Google Sign-In support
- Saved customer profile and address
- Checkout with Cash on Delivery
- Online UPI/card payment support through Razorpay
- View previous orders
- Order confirmation and status tracking
- Submit customer feedback

### Admin Features

- Secure admin login
- Dashboard with business statistics
- Add daily stock entries
- Manage product details
- Edit and delete products
- Track product quantities
- View and manage customer orders
- Update order status
- Monitor today's sales
- View low-stock products
- Review customer feedback

## 📸 Screenshots

### Homepage
![AL AZEEM Store Homepage](home-page.png.png)

### Shopping Cart
![Shopping Cart](cart.png.png)

### Admin Dashboard
![Admin Dashboard](admin-dashboard.png.png)

### Footer
![Footer](footer.png.png)

## 🛠️ Technology Stack

### Frontend

- HTML5
- CSS3
- JavaScript

### Backend

- Node.js
- Express.js
- REST APIs

### Database

- Supabase
- PostgreSQL

### Authentication & Services

- JWT Authentication
- Google OAuth
- Razorpay
- Resend

### Deployment

- Vercel

## 🏗️ Application Architecture

```text
Customer / Admin
       ↓
Frontend
       ↓
Express.js Backend
       ↓
REST APIs
       ↓
Supabase PostgreSQL Database
